type tc_type = Value of Types.value_type | Ret of Types.value_type
[@@deriving show]
...
let create () =
  let state =
    { te = Hashtbl.create guessed_max_var_count_per_scope; parent = None }
  in
...
let extend_env (state : t) =
  { te = Hashtbl.create guessed_max_var_count_per_scope; parent = Some state }
...
let get_local_decls = function
  | Ast.Sequence stmts ->
      ...
      |> List.map (function
           ...
           | Ast.Const { sym; declared_type; _ } -> (sym, (declared_type, false))
           | Ast.Fun { sym; declared_type; _ } -> (sym, (declared_type, false))
...
let put_decls_in_table decls te =
  List.iter
    (fun (sym, (typ, is_mutable)) -> Hashtbl.replace te sym (typ, is_mutable))
    decls
...
let rec type_ast ast_node state : tc_type =
  ...
  | Cond { pred; cons; alt } -> (
      ...
      | Value other ->
          failwith
            ("Condition must be boolean. Actual type: "
      ...
      let _ =
        ...
        | Some (other, is_mutable) ->
            if is_mutable then ()
              ...
              failwith
                ("Expected mutable reference type, got: "
                ^ Types.show_value_type other)
      ...
      | Some (typ, _) -> (
          ...
          | Value actual_type ->
              if are_types_compatible typ actual_type then
                Value Types.TUndefined
              else failwith (make_type_err_msg typ actual_type)
