...
let builtin_list = ref [
  { 
    name = "println"; 
    id = 0; 
    param_types = [[Types.TString; Types.TInt; Types.TFloat; Types.TBoolean]];
...
let is_builtin_name name =
  List.exists (fun b -> b.name = name) !builtin_list
...
let validate_builtin_args builtin_id arg_types =
  ...
  if List.length arg_types <> List.length param_types then
    ...
    List.for_all2 (fun arg_type param_types -> 
      is_valid_param_type param_types arg_type
